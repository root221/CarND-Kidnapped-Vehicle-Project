...
inline double gaussian(double x,double y,double mu_x,double mu_y,double sigma_x,double sigma_y){
	double a = - ( (x-mu_x) * (x-mu_x) / (2*sigma_x*sigma_x) + (y-mu_y) * (y-mu_y) / (2*sigma_y*sigma_y) );
	double b = exp(a);
	return b/(2*M_PI*sigma_x*sigma_y);
}
